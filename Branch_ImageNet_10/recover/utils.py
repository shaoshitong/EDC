...
class ConvFeatureHook():
    def __init__(self, module=None, save_path="./", name=None, gpu=0, training_momentum=0.4,
    ...
    def save(self):
        npz_file = {"running_dd_var": self.running_dd_var.cpu().numpy()/self.total_number if isinstance(self.running_dd_var,
                                                                                      torch.Tensor) else self.running_dd_var/self.total_number,
                    "running_dd_mean": self.running_dd_mean.cpu().numpy()/self.total_number if isinstance(self.running_dd_mean,
                                                                                        torch.Tensor) else self.running_dd_mean/self.total_number,
                    "running_patch_var": self.running_patch_var.cpu().numpy()/self.total_number if isinstance(self.running_patch_var,
                                                                                            torch.Tensor) else self.running_patch_var/self.total_number,
                    "running_patch_mean": self.running_patch_mean.cpu().numpy()/self.total_number if isinstance(self.running_patch_mean,
                                                                                              torch.Tensor) else self.running_patch_mean/self.total_number}
    ...
    def pre_hook_fn(self, module, input, output):
        ...
        self.running_dd_var += (dd_var*bs)
        self.running_dd_mean += (dd_mean*bs)
        self.running_patch_var += (patch_var*bs)
        self.running_patch_mean += (patch_mean*bs)
        self.total_number += bs
...
class PreImgPathCachewithClass(ImageNetwithClass):
    def __init__(
            self,
            root,
            transforms,
        ...
        super(PreImgPathCachewithClass, self).__init__(root,transform=transforms,nclass=nclass)
